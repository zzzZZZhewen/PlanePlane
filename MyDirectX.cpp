...
void Sprite_Animate(int &frame, int startframe, int endframe, int direction, DWORD &starttime, int delay, DWORD now, bool looping)
{
		if (!looping && frame == endframe) return;
...
void Sprite_Transform_Draw(LPDIRECT3DTEXTURE9 image, SPRITE sprite, D3DCOLOR color)
{
		//create a scale vector
		D3DXVECTOR2 scale(sprite.scaling, sprite.scaling);

		//create a translate vector
		D3DXVECTOR2 trans(sprite.x, sprite.y);

		//set center by dividing width and height by two
		D3DXVECTOR2 center((float)(sprite.width * sprite.scaling) / 2, (float)(sprite.height * sprite.scaling) / 2);

		//create 2D transformation matrix
		D3DXMATRIX mat;
		D3DXMatrixTransformation2D(&mat, NULL, 0, &scale, &center, sprite.rotation, &trans);

		//tell sprite object to use the transform
		spriteobj->SetTransform(&mat);

		//calculate frame location in source image
		int fx = (sprite.frame % sprite.columns) * sprite.width;
		int fy = (sprite.frame / sprite.columns) * sprite.height;
